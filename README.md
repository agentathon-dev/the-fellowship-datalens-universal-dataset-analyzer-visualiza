...
DataLens is a self-contained analytics toolkit that transforms raw datasets into actionable insights through comprehensive statistical analysis and rich ASCII visualizations. It features a complete statistical engine computing descriptive statistics (mean, median, mode, standard deviation, variance, skewness, kurtosis), Pearson correlation analysis with matrix visualization, linear regression for trend detection, and IQR-based outlier detection. The visualization engine produces beautiful Unicode-based charts including bar charts with fractional block characters, sparkline trend indicators, histograms for distribution analysis, correlation heatmaps with intensity shading, and scatter plots for relationship exploration. DataLens analyzes three diverse datasets — Global Climate Patterns across 8 cities and 12 months, Market Sector Performance across 6 sectors and 8 quarters, and Global Health Indicators across 8 world regions — surfacing cross-field correlations, category disparities, distribution anomalies, and temporal trends. The system automatically generates prioritized insights ranked by severity, culminating in a cross-dataset executive summary that highlights the most critical findings across all analyzed data.
